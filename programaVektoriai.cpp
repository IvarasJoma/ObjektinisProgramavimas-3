...
int main(){
    ...
    while (true) {
        ...
        if (pasirinkimas == 4){
            ...
            std::vector<std::string> meniuRusiavimo = {"Pasirinkite, pagal ką norite rūšiuoti studentus:", "1 - vardą", "2 - pavardę", "3 - galutinį pažymį pagal vidurkį", "4 - galutinį pažymį pagal medianą"};
            int pasirinkimasRusiavimo = nuskaitytiMeniuPasirinkima(meniuRusiavimo);
            if (pasirinkimasRusiavimo == 1) std::sort(studentuSarasas.begin(), studentuSarasas.end(), lygintiElementusPagalReiksme(&StudentasVektorius::Vardas));
            if (pasirinkimasRusiavimo == 2) std::sort(studentuSarasas.begin(), studentuSarasas.end(), lygintiElementusPagalReiksme(&StudentasVektorius::Pavarde));
            if (pasirinkimasRusiavimo == 3) std::sort(studentuSarasas.begin(), studentuSarasas.end(), lygintiElementusPagalReiksme(&StudentasVektorius::galutinisRezultatasPagalVidurki));
            if (pasirinkimasRusiavimo == 4) std::sort(studentuSarasas.begin(), studentuSarasas.end(), lygintiElementusPagalReiksme(&StudentasVektorius::galutinisRezultatasPagalMediana));
