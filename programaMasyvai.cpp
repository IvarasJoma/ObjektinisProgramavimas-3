...
int main() {
    ...
    while (true) {
        ...
        if (meniu == 2) maksimalusNDKiekis = nuskaitytiNeneigiamaSveikajiSkaiciu("Įveskite maksimalų galimą namų darbų pažymių kiekį ir paspauskite ENTER: ");
        while (true) {
            ...
            if (meniu == 2) {
                ...
                while (true) {
                    ...
                    if (nuskaitytiSveikajiSkaiciu(ivestis, reiksme) && reiksme >= 1 && reiksme <= 10) {
                        uztikrintiNamuDarbuMasyvoTalpa(studentas->namuDarbuTarpiniaiRezultatai, studentas->namuDarbuTalpa, studentas->namuDarbuKiekis, studentas->namuDarbuKiekis + 1);
